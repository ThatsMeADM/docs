...
| Header | Purpose |
|---|---|
| `X-TM-Signature` | HMAC-SHA256 signature (see format below) — **verify this before doing anything else** |
| `X-TM-Event` | Event type (`certificate.issued`, `certificate.accepted`, etc) — handy for routing without parsing the body |
| `X-TM-Delivery` | Unique delivery ID — use for idempotency / dedup on your side (we may retry on transient failures) |
